...
| Первая загрузка | 163 КБ: оболочка и бипы. Демо-музыка (11 МБ) — лениво, по первому жесту |
...
└── sounds/
    ...
    ├── demo_work.m4a   # Демо-музыка фазы Работа   ─┐ 11 МБ на троих:
    ├── demo_relaxe.m4a # Демо-музыка фазы Отдых     ─┤ не в precache,
    └── demo_fin.m4a    # Демо-музыка фазы Финиш     ─┘ грузятся лениво
```
...
Service Worker кладёт в precache только оболочку и четыре коротких бипа — **163 КБ**.
Демо-музыка весит 11 МБ на три трека, и до 10.09.2026 она лежала в том же precache:
качалась при установке SW, то есть на первом же заходе, до того как человек что-либо
нажал. Вторым потоком её же тянул `installDemoMusicIfNeeded()` в `IndexedDB` — на
мобильной сети это были десятки мегабайт в фоне ради музыки, которую могут никогда
не включить.

Теперь загрузка начинается по первому жесту (`_preUnlock`, там же, где разблокируется
аудио) и идёт последовательно, чтобы не занимать канал целиком. Точки, где музыка
действительно нужна — `_loadMusicBuffer()` и превью в редакторе, — ждут её через
`ensureDemoMusic()`. В офлайне после первого захода ничего не меняется: файлы лежат
и в `IndexedDB`, и в runtime-кэше Service Worker.

Если добавляете что-то тяжёлое — не кладите это в `ASSETS` в `sw.js`. Всё, что там
перечислено, скачивается до первого взаимодействия.
